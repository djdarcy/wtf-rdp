...
if (-not $ModulePath) { throw "WtfRdp.Sessions module not found -- run 'rdp install' first." }
...
if (-not ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole(
    ...
    throw "Administrator required (the rescue runs as SYSTEM). Re-run from an elevated shell."
}
if ($SessionId -le 0) {
    $st = Find-Stranded
